...
contract Tie32 is ERC20 {
    ...
    function _msgSender() private view returns (address) {return msg.sender; } //since they changed this bitch already countless times, better keep it in one place for easier update
    ...
    function balanceOf(address wallet) external view override returns(uint256) { return _balances[wallet]; } //need to check if blacklisted, then return backup balance since the original is redistributed imo dunno damn this shit hard af xd
    ...
    function _approve(address keeper, address spender, uint256 amount) private { //couldn`t write owner instead of keeper, we better change that either or keep it like this
